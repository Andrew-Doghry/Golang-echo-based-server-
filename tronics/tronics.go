...
func splice(s []body, index int) []body {
	return append(s[:index], (s[index+1:])...) //spread the last value
...
//		ele, _ := strconv.Atoi(ctx.Param("id"))
//		if ele <= len(products) {
//			ctx.JSON(http.StatusOK, products[ele-1])
//			return nil
//		} else {
...
var port = os.Getenv("My_App_Port")

// init here starts before execution of any other functions in the same
// package
func init() {
	err := cleanenv.ReadEnv(&cfg)
	// fmt.Printf("config => %v", cfg)
	if err != nil {
		e.Logger.Fatal("unable to load the configuration")
	}
}
func listProductsMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		if ctx.Request().Method == "GET" {
			fmt.Println("inside middleware func\n000")

		}
		return next(ctx)
	}
}
func lPsMiddlewareDo(next echo.HandlerFunc) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		if ctx.Request().Method == "GET" {
...
func redirectUrls(next echo.HandlerFunc) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		ctx.Request().URL.Path = "/andrew"
		// fmt.Print(ctx.Request())
		return next(ctx)
	}
}
func lpMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		return next(ctx)
	}
...
type Pvalidator struct {
	Validaotr *validator.Validate
}

func (p *Pvalidator) Validate(i interface{}) error {
	return p.Validaotr.Struct(i)
}

func Start() {
	var v = validator.New()
	e.Validator = &ProductValidator{validator: v}
	// e.Use(listProductsMiddleware)
	// e.Pre(listProductsMiddleware)
	// e.Pre(redirectUrls) //pre before calling the method
	// user while calling the method
	e.Use()
	e.Pre(middleware.RemoveTrailingSlash()) //it removes the / at the end
	e.GET("/product", GetProductByBody, redirectUrls, middleware.BodyLimit("1K"))
	// statusCode request entity too large B byte G
	e.GET("/products", ListProducts, listProductsMiddleware, lPsMiddlewareDo)
	e.GET("/products/:id", GetProductByBody)
	// M megabyte K killoBYte
	e.POST("/product", PostProduct)
	e.PUT("/products/:id", UpdateProduct)
	e.DELETE("/products/:id", DeleteProduct)
	e.Logger.Print("listening on the port ", cfg.Port)
	e.Logger.Fatal(e.Start(fmt.Sprintf(":%v", cfg.Port))) //as start returns error the logger will print it before exit
}
